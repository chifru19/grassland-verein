 ...
 * - Dynamic Gallery (General, Meetings, Kids, Events)
 * - Video & Image handling
 * - GitHub Pages deployment-optimized pathing
 * * Architecture:
 * This component handles state for language switching, form submission,
 * and responsive rendering. All media paths are processed via the 'p' helper
 * to ensure compatibility with sub-directory hosting.
 */
...
export default function App() {
  ...
  // Helper: Ensures public assets are found correctly on GitHub Pages
  const p = (path) => `${process.env.PUBLIC_URL}${path}`;

  // --- 2. DATA REPOSITORIES (Using p() helper) ---
  ...
  const handleRegister = (e) => {
    e.preventDefault();
    alert(`Thank you, ${formData.name}! Registration confirmed.`);
      ...
      <main>
        ...
        <section id="meetings" style={styles.section}>
          ...
          <div style={styles.grid}>
            {meetingGalleryImages.map((src, i) => <div key={i} style={styles.card}><img src={src} style={styles.media} alt="Meeting" /></div>)}
          </div>
        </section>
        ...
        <section id="gallery" style={styles.section}>
          ...
          <div style={styles.grid}>
            {mainGalleryImages.map((src, i) => <div key={i} style={styles.card}><img src={src} style={{ width: '100%', height: '280px', objectFit: 'cover' }} alt="Gallery" /></div>)}
          </div>
        </section>
